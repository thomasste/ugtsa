...
void ComputationGraph::run_batch() {
    ...
    // collect inputs
    std::vector<std::vector<int>> transformations_input_count;
    std::vector<std::vector<std::vector<int>>> transformations_input_buffers;

    for (auto &transformation : transformations) {
        std::vector<std::vector<int>> transformation_intput_buffers;
        for (int i = 0; i < transformation.inputs.size(); i++) {
            transformation_intput_buffers.push_back({});
        }
    ...
    for (int transformation_index = 0; transformation_index < transformations.size(); transformation_index++) {
        ...
        if (transformations_input_count > 0) {
            ...
            for (int input_index = 0; input_index < transformation.inputs.size(); input_index++) {
                ...
                auto &input_shape = transformations.input_shapes[input_index];
    ...
    for (int transformation_index = 0; transformation_index < transformations.size(); transformation_index++) {
        ...
        tensorflow::Tensor tensor(DT_INT64, tensorflow::TensorShape({seed.size()}));
        for (int i = 0; i < seed.size(); i++) {
            tensor.flat<long long>()(i) = seed[i];
        }
        feed_dict.push_back(input_name, tensor);
    }

    {
        tensorflow::Tensor tensor(tensorflow::DataType::DT_BOOL, tensorflow::TensorShape({}));
        training.scalar<bool>()(0) = training;
        feed_dict.push_back("training", tensor);
    }
    ...
    for (int transformation_index = 0; transformation_index < transformations.size(); transformation_index++) {
        if (transformations_input_count[transformation_index] > 0) {
            fetch_outputs.push_back(transformation.output);
        }
    }
